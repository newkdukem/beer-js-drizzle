...
export const userBeersBeer = pgTable(
  'user_beers_beer',
  {
    userId: uuid().notNull(),
    beerId: uuid().notNull()
  ...
  table => [
    index('IDX_3b907db6fc5443bd530ce88b6c').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops')
    ),
    index('IDX_5d9e974c43d8f02baaa91952c6').using(
      'btree',
      table.beerId.asc().nullsLast().op('uuid_ops')
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.userId, table.beerId],
      name: 'PK_de3d33a409946ce03876dc9f858'
    })
  ]
);

export const userPubsPub = pgTable(
  'user_pubs_pub',
  {
    userId: uuid().notNull(),
    pubId: uuid().notNull()
  ...
  table => [
    index('IDX_3b32b8b64bd3ab48d1f1e9b204').using(
      'btree',
      table.pubId.asc().nullsLast().op('uuid_ops')
    ),
    index('IDX_9f8d5a27df7525a4213e577942').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops')
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.userId, table.pubId],
      name: 'PK_063ab6a7ec20dd1fdb51d808d5d'
    })
  ]
);

export const pubBeersBeer = pgTable(
  'pub_beers_beer',
  {
    pubId: uuid().notNull(),
    beerId: uuid().notNull()
  ...
  table => [
    index('IDX_8abfc04bf13e6e4fa0397c8850').using(
      'btree',
      table.beerId.asc().nullsLast().op('uuid_ops')
    ),
    index('IDX_d66767a5f9a311713f25c97610').using(
      'btree',
      table.pubId.asc().nullsLast().op('uuid_ops')
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.pubId, table.beerId],
